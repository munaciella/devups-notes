...
Supertest will be responsible for attaching a listener to our Server/App. The application will make requests to the database via a model. The connection will need to know which PGDATABASE, in the example below this is done via a env variable. 

```json 
{
  "scripts": {
    "test": "PGDATABASE=database_name jest"
  }
}
```
...
To fix this, we can re-seed the database before we run the test suite. One way to do this is to use the jest `beforeEach` hook to run our seed function before each test. By requiring in our seed function and passing in our test data before each test is run, we ensure that our database is re-seeded and any destructive changes (i.e. a POST/PATCH/DELETE request) won't affect our other tests. 
