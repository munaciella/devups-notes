...
- Character
  - `CHAR(n)` is the fixed-length character string. If you insert a string that is shorter than the length of the column (n), PostgreSQL pads the spaces. If you insert a string that is longer than the length of the column, PostgreSQL will issue an error.
...
The semi-colon ; is used in SQL to denote the end of a statement or query. Missing a semi-colon will lead to SQL thinking that your statement or query leads into the next one, which will usually cause bugs.
