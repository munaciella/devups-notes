...
## 2. Different Errors & Responses that should be sent
...
## 5. Generic 404 errors

While Express does have its own in built 404 handler, if you wanted to control what response your API is sent back you can handle your own 404 errors. You can do this using `app.all()`, which will route any type of request (POST, PATCH, GET, etc.), and a wildcard `*` which will match any route name.
...
In this case we don’t need `err` or `next` because it is handling all requests to an endpoint that isn’t already being routed, and doesn’t perform any operations that could result in errors themselves.