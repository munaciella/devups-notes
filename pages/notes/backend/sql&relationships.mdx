# SQL - Foreign Keys, Joins & Aggregate Functions
...
In the below tables the `favourite_animal_id` column in the `devups` table is a `FOREIGN KEY` that references the `PRIMARY KEY` of the `animals` table (`animal_id`).

`devups`:

| devups_id | devups | favourite_animal_id |
| ------------: | ---------- | ------------------: |
|             1 | Joey        |                   2 |
|             2 | Moe       |                   4 |
|             3 | Bart       |                   2 |
...
The above creates a **one-to-many** relationship between `devups` and `animals`; where many devups may have the same favourite animal.

To implement the above relationship between two tables, the `devups` table must be given the `favourite_animal_id` field in the `CREATE TABLE` statement. This will be of type `INT` to match the `favourite_animal_id` field in the `animals` table.

In order to have primary keys auto generated as incrementing integers, `SERIAL` can be used in place of `INT` when creating a table's primary key (e.g. `devups_id SERIAL PRIMARY KEY,`).
...
In order for the `devups` table to reference the `animal_id` from the `animals` table, the `animals` table must be created before the `devups` table, or it will throw an error from trying to reference a table, and column that does not exist.

```sql
-- example.sql
DROP DATABASE IF EXISTS devups_db;
CREATE DATABASE devups_db;
...
In order for the `devups` rows to be created with a reference to a specific `animal` from the `animals` table, the `animal` must exist in the `animals` table, or it will throw an error from trying to reference an `animal_id` that does not exist.
...
FROM devups
JOIN animals
ON devups.favourite_animal_id = animals.animal_id;
```

The above would return the following result:

| devups_id | devups | favourite_animal_id | animal_id | animal_name | number_of_legs |
| ------------: | ---------- | ------------------: | --------: | ----------- | -------------: |
|             1 | Joey        |                   2 |         2 | Butterfly   |              6 |
|             2 | Moe       |                   4 |         4 | Koala       |              4 |
|             3 | Bart       |                   2 |         2 | Butterfly   |              6 |
|             4 | Lisa       |                   3 |         3 | Unicorn     |              4 |
...
E.g. a devups can have many skills, and a skill can be had by many devups.

### A Bad Example:

Using foreign keys could lead to a table structure something like the below. However, this would require the `devups` table to be constantly updated if a new column was required because a devups had exceeded the current maximum number of skills:

`devups`:

| devups_id | devups | favourite_animal_id | skill_id_1 | skill_id_2 | ...more columns!? |
| ------------: | ---------- | ------------------: | ---------: | ---------: | ----------------- |
|             1 | Joey        |                   2 |          1 |       null |                   |
|             2 | Moe       |                   4 |          3 |          3 |                   |
|             3 | Bart       |                   2 |          5 |       null |                   |
...
| devups_skills_id | devups_id | skill_id |
| --------------------: | ------------: | -------: |
|                     1 |             1 |        1 |
|                     2 |             1 |        4 |
|                     3 |             1 |        7 |
...
The `devups_skills` table (named after the two tables that it forms a junction between) holds the information needed to store and maintain the **many-to-many** relationships between the two tables.
...
CREATE TABLE devups_skills (
   id SERIAL PRIMARY KEY,
  devups_id INTEGER REFERENCES devups(devups_id),
  skill_id INTEGER REFERENCES skills(skill_id)
);

INSERT INTO devups_skills
  (devups_id, skill_id)
VALUES
  (1,1), (1,4), (1,7), (1,6), (2,5), (2,2), (2,1), (3,3), (3,1), (3,4), (4,5), (4,4), (4,7);

SELECT * FROM devups_skills
JOIN devups ON devups_skills.devups_id = devups.devups_id
JOIN skills ON devups_skills.skill_id = skills.skill_id;
```
...
Consider building a query to display each animal's name and the total number of devups who have selected the animal as their favourite.
...
SELECT *
FROM animals
JOIN devups ON devups.favourite_animal_id = animals.animal_id;
```

| animal_id | animal_name | number_of_legs | devups_id | devups | favourite_animal_id |
| --------: | ----------- | -------------: | ------------: | ---------- | ------------------: |
|         2 | Butterfly   |              6 |             1 | Joey        |                   2 |
|         4 | Koala       |              4 |             2 | Moe       |                   4 |
|         2 | Butterfly   |              6 |             3 | Bart       |                   2 |
|         3 | Unicorn     |              4 |             4 | Lisa       |                   3 |
...
SELECT *
FROM animals
LEFT JOIN devups ON devups.favourite_animal_id = animals.animal_id;
```
...
2. Having a row for every unique animal-devups combination isn't what we want. The rows created can be "squashed" together using `GROUP BY`, which will take some column to group the rows together with. Note: we can only `SELECT` columns that share the same values across the rows being grouped together.
...
At this point, the results look as if we are only querying the animals table. However, we now have access to the data in the devups table.

3. We are now able to use an aggregate function to count the number of devups associated with each animal. To do this, we count the `devups_id`s associated with each animal. This will show up as a new column on the result of the query called `count`.

```sql
SELECT animals.*, COUNT(devups_id)
FROM animals
LEFT JOIN devups ON devups.favourite_animal_id = animals.animal_id
GROUP BY animal_id;
```
...
```sql
SELECT animals.*, COUNT(devups_id) AS number_of_fans
FROM animals
LEFT JOIN devups ON devups.favourite_animal_id = animals.animal_id
GROUP BY animal_id;
```
...
In the following example, if one of the animals were deleted, any devups whose `favourite_animal_id` referenced the deleted animal would now have that field set to `NULL`.
