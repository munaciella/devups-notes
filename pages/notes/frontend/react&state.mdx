...
In the example above name is set to it's initial value of 'Moe' and can be used just as any other variable would.
...
const App = () => {
  ...
  return (
    <div>
      <h1>Hello {name}</h1>
      <button onClick={() => setName('Lisa')}>Say hi to Lisa</button>
      <button onClick={() => setName('Joey')}>Say hi to Joey</button>
    </div>
  );
};
```
...
If we pass onClick `(event) => setName('Lisa')`, this function will be called with a synthetic `event` when triggered that is created by React (but for all intended purposes, the same as the 'real' event).
...
If we were to pass `setName('Lisa')` to the `onClick`, the `onClick` is no longer being passed a _function_, but a _function invocation_. So as soon as this component renders for the first time, this function will be called.
...
These were introduced in React v16.8 and weren't available before then.
