...
OOP exploits the fact that objects in JavaScript can hold data in properties and hold 'behaviours' (functions) as methods. The `joeysAccount` object below holds the data of the user's `username` and `basket`, and also has a method `addToBasket` that adds an item to the `basket` array.
...
const joeysAccount = createUser("joey");
...
```js
console.log(joeysAccount.addToBasket === anatsAccount.addToBasket); // FALSE
```
...
joeysAccount.addToBasket("sandwiches");
```

In the example above, `joeysAccount` is the 'owner' of the function `addToBasket`, so any occurences of `this` within that function act as placeholders for the `joeysAccount` object.
...
```js
addToBasket.call(joeysAccount, "sandwiches");
addToBasket.apply(joeysAccount, ["sandwiches"]);
```

Both of the above invoke the function `addToBasket`, setting the value of `this` to be `joeysAccount`. The difference between `call` and `apply` is only in how you pass in arguments to `addToBasket` - `call` takes multiple arguments, whereas `apply` takes an array of arguments.

Another method available is `bind`.

```js
const addTojoeysBasket = addToBasket.bind(joeysAccount);
```

Invoking bind returns a new function - in the example above, it returns a copy of `addToBasket` where the value of `this` is **always** going to be the `joeysAccount` object. A bound function cannot have the `this` value changed.
