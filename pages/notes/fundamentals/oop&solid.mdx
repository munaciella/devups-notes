...
We currently can use all the Pokemon subclasses in the same way, they all have the same methods, this is intentional, we should be able to use a Bulbasaur in the same way we interact with any given Pokemon. This could become an issue if we created a Pokemon that was able to use multiple moves ...
​
...
​Another example taken from [Stack Exchange](https://softwareengineering.stackexchange.com/questions/170222what-can-go-wrong-if-the-liskov-substitution-principle-is-violated)



## I - Interface Segregation Principle (ISP)

The **Interface Segregation Principle (ISP)** is a concept in object-oriented programming that suggests classes should not be forced to implement methods they don't need. Instead of creating one large interface or class with many methods, it's better to split it into smaller, more focused ones. This way, each class can only implement the methods it actually uses.

**Example Without ISP** (Not Ideal)

In this example, the `Pokemon` class has several methods. The `Magikarp` class extends `Pokemon` and is forced to implement all methods, even those it doesn't need.

```javascript
// A general Pokemon class
class Pokemon {
  constructor(name) {
    this.name = name;
  }
...
// Magikarp class inherits unnecessary methods
class Magikarp extends Pokemon {
  ...
  attack() {
    console.log(`${this.name} splashes around uselessly...`);
  }

  // Magikarp doesn't evolve or swim, but must implement these methods
  evolve() {
    throw new Error("Magikarp can't evolve (yet)!");
  }

  swim() {
    throw new Error("Magikarp doesn't swim in this context!");
  }
}
```

**Example With ISP** (Better Practice)

With ISP, we split the `Pokemon` class into smaller, more focused classes. Each Pokémon class now only implements the methods it needs.

```javascript
// Base class for attackable Pokémon
class Attackable {
  constructor(name, attackDamage, hitPoints) {
    this.name = name;
  ...
  attack() {
    console.log(`${this.name} attacks with damage of ${this.attackDamage}`);
  }
}

// Base class for Pokémon with special moves
class Movable {
...
// Pikachu needs to attack and use moves, so it extends both
class Pikachu extends Attackable {
  constructor(name) {
    super(name, 500, 300);
...
// Snorlax only attacks, so it extends Attackable
class Snorlax extends Attackable {
  constructor(name) {
    super(name, 800, 1200);
  }
}
...
The Interface Segregation Principle helps us design classes that only implement the methods they need, making the code cleaner and easier to maintain.
​
